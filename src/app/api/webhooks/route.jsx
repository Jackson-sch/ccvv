...
export async function POST(req) {
  ...
  if (!WEBHOOK_SECRET) {
    throw new Error(
      "Por favor, añade WEBHOOK_SECRET desde el Dashboard de Clerk a .env o .env.local"
    );
  }
  ...
  try {
    ...
    const email =
      email_addresses.length > 0 ? email_addresses[0].email_address : undefined;
    const status =
      email_addresses.length > 0
        ? email_addresses[0].verification.status
        : undefined;
    const clerkId = id;
    ...
    if (eventType === "user.created") {
      ...
      await User.updateOne({ clerkId }, { name, imageUrl, email, status, ...userData });
